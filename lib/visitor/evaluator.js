...
Evaluator.prototype.visitFunction = function(fn){
  ...
  var sourcemaps = this.options._sourcemaps;
  if (sourcemaps && sourcemaps.calls && this.options.filename === fn.filename) {
...
Evaluator.prototype._logInnerCalls = function (node) {
  ...
  if (!nodes) return;
  var self = this;
  var sourcemaps = this.options._sourcemaps.calls;
  return nodes.forEach(function (n) {
    if (n.nodeName === 'call') {
      var fn = self.lookup(n.name);
      if (fn) {
        sourcemaps.push([n.lineno - 1 || 0, n.name, fn.filename, fn.lineno]);
      }
    }
...
Evaluator.prototype.visitCall = function(call){
  ...
  // User-defined
    ...
    var sourcemaps = this.options._sourcemaps;
    if (sourcemaps && sourcemaps.calls && call.filename === this.filename) {
      sourcemaps.calls.push([
        call.lineno - 1 || 0, call.name, fn.filename, fn.lineno
      ]);
    }
